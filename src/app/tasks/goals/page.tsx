...
export default async function GoalsPage() {
  ...
  const [{ data: goals }, { data: tasks }] = await Promise.all([
    supabase.from('goals').select('id,title,description,status,target_date,term,postponement_count').eq('workspace_id', activeWorkspace.id).neq('status', 'achieved').order('target_date'),
    supabase.from('tasks').select('id,title,status,due_date,due_time,goal_id').eq('workspace_id', activeWorkspace.id).neq('status', 'completed').order('created_at'),
  ])
